...
func CheckOneRule(request *models.ReqParameters, rule *models.Rule) bool {
	...
	*/
	...
	// 4系统版本
	if !(rule.MinOsApi <= request.OsApi && request.OsApi <= rule.MaxOsApi) {
		return false
	}
	...
	/** 6白名单
	 * 查找是否出现在白名单 出现返回 true，否则返回 false
	 */
	for _, deviceId := range rule.DeviceIdList {
...
func DeviceId2IsNotGreaterDeviceId1(deviceId1 string, deviceId2 string) bool {
	 ...
	 * output : flase :: deviceId = deviceId2
	 *
